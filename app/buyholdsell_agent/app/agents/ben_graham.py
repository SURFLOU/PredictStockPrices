...
class BenGrahamSignal(BaseModel):
    signal: Literal["buy", "sell", "hold"] = Field(description="Investment signal")
...
def create_ben_graham_agent(model) -> Agent:
    ben_graham_agent = Agent(
        model=model,
        output_type=BenGrahamSignal,
        system_prompt=(
            """
            ...
            Return a rational recommendation: buy, sell, or hold, with a confidence level (0-100) and thorough reasoning.
            """
        ),
    )

    @ben_graham_agent.tool
    async def analyze(ctx: RunContext, input_text: str) -> BenGrahamSignal:
        human_prompt = f"""
...
"""
        return await ctx.llm.chat(
            ...
            output_model=BenGrahamSignal,
        )

    return ben_graham_agent
