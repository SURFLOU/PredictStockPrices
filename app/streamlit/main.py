...
def fetch_all_tickers():
    response = requests.get(f"{API_URL}/gettickers?api-key={API_KEY}")
    if response.status_code == 200:
        return response.json()
    return []

def fetch_stock_data(symbol):
    response = requests.get(f"{API_URL}/historical/{symbol}?api-key={API_KEY}")
    if response.status_code == 200:
        return response.json()["historical_data"]
    return []

def fetch_stock_name(symbol):
    response = requests.get(f"{API_URL}/getstockname/{symbol}?api-key={API_KEY}")
    if response.status_code == 200:
        return response.json()
    return []
